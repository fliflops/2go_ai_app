...
export const invoiceExtractionPrompt = (invoice: string) => {
   return `Extract structured data from Philippine sales invoices into JSON format following BIR requirements.
		...
		**document_control_type**: "ATP" (Authority to Print), "PTU" (Permit to Use), "OCN" (Official Control Number), "ACCN" (Accounting Control Number), or null
		**document_control_number**: Control number if present
		**document_control_date**: Date issued (YYYY-MM-DD) if shown
